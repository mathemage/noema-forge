...
This is a **reflection practice, not therapy**. It is not a therapist, it does not diagnose, and it
is not a substitute for care. The honest ceiling is part of the design: expressive writing returns
roughly a quarter of the effect of psychotherapy, and unguided self-help underperforms the same
programme with a human alongside it. Saying so is not modesty; a tool that oversells itself sets
you up to conclude *you* failed when the tool underperformed.

Three consequences run through the product. Crisis resources are hard-coded per locale and never
generated by a model. Severity check-ins store a number and never interpret it. And nothing you
write is filtered, blocked, or refused, because guardrails that block a person mid-sentence break
the one thing a private journal is for.
